...
export default function EmployeesPage() {
  ...
  const list = useCollection(['company', company.id, 'employees', status], (cursor, signal) =>
  ...
  return (
    <>
      <PageHeading
        title="Employees"
        description="Employee records are separate from application accounts."
        action={
          editable ? (
      ...
      <Filter
        ...
        value={status}
        options={['active', 'leave', 'terminated']}
        onChange={setStatus}
      />
      <Collection {...list.props}>
        ...
        >
          {list.items.map((employee) => (
            <Paper key={employee.id} variant="outlined" sx={{ p: 3, minWidth: 0 }}>
              <Stack direction="row" spacing={2} sx={{ justifyContent: 'space-between', mb: 2 }}>
                <Typography variant="h5" component="h2" noWrap title={employee.full_name}>
                  {employee.full_name}
                </Typography>
                <Chip size="small" label={employee.status} variant="outlined" />
              </Stack>
              <Typography color="text.secondary" noWrap title={employee.job_title ?? ''}>
                {employee.job_title || 'No job title'}
              </Typography>
              <Typography variant="body2" noWrap title={employee.work_email ?? ''}>
                {employee.work_email || 'No work email'}
              </Typography>
              <Typography variant="body2">{employee.work_phone || 'No work phone'}</Typography>
              <Typography variant="body2" sx={{ mt: 2 }}>
                Start: {employee.start_date} · End: {employee.end_date || '—'}
              </Typography>
              <Typography
                variant="caption"
              ...
              {editable ? (
                <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
                  <Button
                    variant="outlined"
                    onClick={() => setAction({ id: employee.id })}
                    aria-label={'Edit ' + employee.full_name}
                  >
                    Edit
                  </Button>
                  <Button
                    onClick={() => setAction({ id: employee.id, remove: true })}
      ...
      {action ? (
        <EntityDialog
          ...
          title={action.remove ? 'Delete employee' : action.id ? 'Edit employee' : 'Add employee'}
          fields={employeeFields}
          resource={resource}
          notice={
            action.remove
              ? 'Permanently delete this employee and their project assignments?'
              : 'Terminating employment removes active project assignments. An end date is required for termination.'
          }
