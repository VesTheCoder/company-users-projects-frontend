...
export default function CompaniesPage() {
  ...
  const list = useCollection(['companies', role], (cursor, signal) =>
    companies.list(cursor, { role }, signal),
  )
  return (
    <>
      <PageHeading
        title="Companies"
        description="Your organizations and the people behind them."
        action={
      ...
      <Collection {...list.props}>
        ...
        >
          {list.items.map((company) => (
            <Paper key={company.id} variant="outlined" sx={{ p: 3, minWidth: 0 }}>
              <Chip size="small" variant="outlined" label={company.current_role} sx={{ mb: 2 }} />
              <Typography variant="h5" component="h2" noWrap title={company.name}>
                {company.name}
              </Typography>
              <Typography
