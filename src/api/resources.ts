...
export const employees = (companyId: string) =>
  resource<Employee, EmployeeInput>('/companies/' + companyId + '/employees')
export const projects = (companyId: string) =>
  resource<Project, ProjectInput>('/companies/' + companyId + '/projects')
export const access = (companyId: string) => {
  const path = '/companies/' + companyId + '/access'
  return {
    path,
    list: (cursor?: string | null, signal?: AbortSignal) =>
      api.request<Page<Access>>(pagePath(path, cursor), { signal }).then((r) => r.data),
    grant: (body: { user_id: string; role: Exclude<Role, 'owner'> }) =>
      api.request<Access>(path, { method: 'POST', body }),
    update: (userId: string, role: Exclude<Role, 'owner'>) =>
      api.request<Access>(path + '/' + userId, { method: 'PATCH', body: { role } }),
    ...
    transfer: (body: { new_owner_user_id: string; previous_owner_role: Exclude<Role, 'owner'> }) =>
      api.request<Access[]>('/companies/' + companyId + '/ownership-transfer', {
        method: 'POST',
        body,
      }),
  }
...
export const assignments = (companyId: string, projectId: string) => {
  ...
  return {
    path,
    list: (cursor?: string | null, signal?: AbortSignal) =>
      api.request<Page<Assignment>>(pagePath(path, cursor), { signal }).then((r) => r.data),
    assign: (employeeId: string) =>
      api.request<Assignment>(path + '/' + employeeId, { method: 'PUT' }),
    remove: (employeeId: string) =>
      api.request<void>(path + '/' + employeeId, { method: 'DELETE' }),
  }
}
