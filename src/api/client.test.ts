...
describe('API boundary', () => {
  it('uses cookies, CSRF and the snapshot ETag and accepts 204', async () => {
    ...
    expect(
      (await client.request('/companies/1', { method: 'DELETE', etag: '"v3"' })).data,
    ).toBeUndefined()
  ...
  it('uses login protection without an existing token', async () => {
    ...
    vi.stubGlobal('fetch', fetcher)
    await new ApiClient('').request('/auth/login', {
      method: 'POST',
      login: true,
  ...
  it('expires the session on 401 and prevents subsequent writes', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(Response.json({ code: 'expired' }, { status: 401 })),
    )
    ...
    await expect(client.request('/companies', { method: 'POST' })).rejects.toMatchObject({
      status: 401,
    })
  })
  ...
  it('maps field pointers, keeps conflict guidance and hides server internals', () => {
    expect(
      new ApiError(422, {
        errors: [{ pointer: '/work_email', code: 'invalid', message: 'Invalid email' }],
      }).fields.work_email,
    ).toBe('Invalid email')
  ...
  it('rejects a response when the session changes during JSON parsing', async () => {
    let resolve!: (value: unknown) => void
    const response = Response.json({})
    const parsing = vi.spyOn(response, 'json').mockImplementation(
      () =>
        new Promise((r) => {
          resolve = r
        }),
  ...
  it('encodes signed cursors and omits empty filters', () => {
    const url = new URL(
      pagePath('/companies', 'a+b/c=', { role: 'viewer', status: '' }),
      'http://localhost',
    )
