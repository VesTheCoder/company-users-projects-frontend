...
describe('domain form contracts', () => {
  ...
  it('requires an end date only for terminated employees', () => {
    expect(validateDates({ full_name: 'A', status: 'terminated', start_date: '2026-01-01', end_date: '' })).toHaveProperty('end_date')
    expect(validateDates({ full_name: 'A', status: 'active', start_date: '2026-01-01', end_date: '2026-02-01' })).toHaveProperty('end_date')
    expect(validateDates({ full_name: 'A', status: 'terminated', start_date: '2026-01-01', end_date: '2026-02-01' })).toEqual({})
  })
  it('rejects reversed or incomplete project dates', () => {
    ...
    expect(validateDates({ start_date: '2026-02-01', end_date: '2026-01-01' })).toHaveProperty('end_date')
  })
  it('sends cleared optional fields as null', () => {
    expect(payload({ name: ' Company ', description: '' })).toEqual({ name: 'Company', description: null })
  })
})
