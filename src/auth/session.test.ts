...
it('restores CSRF in memory, signs in and clears cached tenant data on logout', async () => {
  ...
  request.mockResolvedValueOnce({ data: { id: 'u', display_name: 'Owner' }, etag: null })
    .mockResolvedValueOnce({ data: { csrf_token: 'restored' }, etag: null })
  await restoreSession()
  expect(token).toHaveBeenCalledWith('restored')
  request.mockResolvedValueOnce({ data: { user: { id: 'u' }, expires_at: 'tomorrow', csrf_token: 'login' }, etag: null })
  await signIn('owner@example.com', 'password')
  expect(request).toHaveBeenLastCalledWith('/auth/login', expect.objectContaining({ login: true, method: 'POST' }))
